...
func TestCreateAsset_Fungible(t *testing.T) {
	...
	a, err := m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{
...
func TestCreateAsset_MissingName(t *testing.T) {
	...
	_, err := m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{TrackingMode: mwanachamaassetmanager.AssetTrackingModeFungible})
...
func TestCreateAsset_InvalidTrackingMode(t *testing.T) {
	...
	_, err := m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{Name: "Mystery", TrackingMode: "bogus"})
	if !errors.Is(err, mwanachamaassetmanager.ErrInvalidAsset) {
		t.Fatalf("expected ErrInvalidAsset, got %v", err)
	}
}
...
func TestCreateAsset_SerializedMissingSerialTag(t *testing.T) {
	...
	_, err := m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{Name: "Laptop", TrackingMode: mwanachamaassetmanager.AssetTrackingModeSerialized})
	if !errors.Is(err, mwanachamaassetmanager.ErrInvalidAsset) {
		t.Fatalf("expected ErrInvalidAsset, got %v", err)
	}
}
...
func TestUpdateAsset_TrackingModeImmutable(t *testing.T) {
	...
	_, err := m.UpdateAsset(ctx, testAgency, a)
...
func TestDeleteAsset_OpenHoldsBlocks(t *testing.T) {
	...
	if err := m.DeleteAsset(ctx, testAgency, a.ID); !errors.Is(err, mwanachamaassetmanager.ErrAssetHasOpenHolds) {
...
func TestListAssets_FilterByCategory(t *testing.T) {
	...
	_, _ = m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{Name: "Rice", TrackingMode: mwanachamaassetmanager.AssetTrackingModeFungible, Category: "grocery"})
	_, _ = m.CreateAsset(ctx, testAgency, mwanachamaassetmanager.Asset{Name: "Bessie", TrackingMode: mwanachamaassetmanager.AssetTrackingModeSerialized, SerialTag: "COW-1", Category: "livestock"})

	groceries, err := m.ListAssets(ctx, testAgency, mwanachamaassetmanager.AssetFilter{Category: "grocery"})
	if err != nil {
