...
| # | Question | Decision |
| - | -------- | -------- |
| 1 | Product boundary | Standalone repo/product. No tier/RLS coupling, no DSN board. Reuses `mwanachama-backend-shared`'s `entitygraph.DataManager` engine and `mwanachama-backend-taskmanager`'s file/schema conventions. **Superseded 2026-09-04**: storage moved off `entitygraph` onto GORM directly, mirroring `mwanachama-backend-actor`'s same-day migration — see `todo_done.md`'s A10 and this repo's `CLAUDE.md`/`doc.go`. |
...
| 4 | Tenancy | Multi-tenant. Free: `entitygraph` already scopes every vertex by `AgencyID` — "owner" (a household, a store, a warehouse operator) maps directly onto Agency, no new plumbing needed. **Superseded 2026-09-04**: dropped along with the entitygraph storage engine it rode on — the GORM migration made this single-tenant instead of re-plumbing `AgencyID` as a column, matching `mwanachama-backend-actor`'s identical call. See `todo_done.md`'s A10. |
